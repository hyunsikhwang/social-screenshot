...
export default function TelegramImageGallery({ imageInfo, imageUrl, postId }: TelegramImageGalleryProps) {
  const [displayMode, setDisplayMode] = useState<"gallery" | "card">("gallery");
  const [selectedUrls, setSelectedUrls] = useState<Set<string>>(() => new Set(imageInfo.imageUrls));
  ...
  const filteredImages = useMemo(() => {
    if (!searchFilter.trim()) return imageInfo.imageUrls;
    const q = searchFilter.toLowerCase();
    return imageInfo.imageUrls.filter((url) => url.toLowerCase().includes(q));
  }, [imageInfo.imageUrls, searchFilter]);
  ...
  return (
    <div className="w-full max-w-3xl bg-slate-950 rounded-2xl p-4 sm:p-6 border border-slate-800 shadow-2xl space-y-5 animate-fade-in" id="telegram-image-gallery-container">
      {/* Top Bar Navigation */}
      <div className="flex flex-wrap items-center justify-between border-b border-slate-800/80 pb-4 gap-3">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-sm font-bold text-white tracking-wide flex items-center gap-1.5">
            🖼️ 텔레그램 첨부 이미지 추출 완료 ({imageInfo.imageUrls.length}개)
          </span>
        </div>
      ...
      <div className="bg-slate-900/90 rounded-xl p-4 border border-slate-800/80 space-y-2 text-xs">
        <div className="flex items-center justify-between border-b border-slate-800 pb-2.5">
          <div className="flex items-center gap-2 min-w-0">
            {imageInfo.authorAvatar && (
              <img
                src={imageInfo.authorAvatar}
                alt="Author Avatar"
                className="w-7 h-7 rounded-full border border-slate-700 object-cover shrink-0"
                referrerPolicy="no-referrer"
              />
            )}
            <span className="font-bold text-slate-100 text-sm truncate">{imageInfo.authorName || "Telegram Channel"}</span>
            {imageInfo.authorHandle && (
              <span className="text-slate-400 text-xs shrink-0">@{imageInfo.authorHandle}</span>
            )}
          </div>
          {imageInfo.views ? (
            <span className="text-slate-400 font-mono text-[11px] shrink-0">
              👁️ {imageInfo.views.toLocaleString()} 회
            </span>
          ) : null}
        </div>
        {imageInfo.tweetText && (
          <p className="text-slate-300 leading-relaxed text-xs pt-0.5 select-all whitespace-pre-wrap">
            {imageInfo.tweetText}
          </p>
        )}
      </div>
      ...
      {displayMode === "gallery" && (
        <div className="space-y-4">
          ...
          <div className="bg-slate-900/80 border border-slate-800 rounded-xl p-3 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              ...
              <div className="text-xs font-medium text-slate-300">
                선택됨: <span className="font-bold text-emerald-400">{selectedUrls.size}</span> / {imageInfo.imageUrls.length}개
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button
                ...
                className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white font-bold rounded-lg text-xs shadow-lg shadow-emerald-600/20 flex items-center gap-2 transition-all cursor-pointer"
              >
                {isZipping ? (
                  <>
          ...
          {/* Search Filter Input (if > 4 images) */}
          {imageInfo.imageUrls.length > 4 && (
            <div className="relative">
              <input
                type="text"
                placeholder="이미지 URL로 검색..."
                value={searchFilter}
                onChange={(e) => setSearchFilter(e.target.value)}
                className="w-full bg-slate-900 border border-slate-800 rounded-lg px-3 py-2 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-emerald-500/60"
              />
            </div>
          )}

          {/* Image Grid Album Matrix */}
          <div className={`grid gap-3 ${filteredImages.length > 2 ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4" : "grid-cols-1 sm:grid-cols-2"}`}>
            {filteredImages.map((imgUrl, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Index Badge */}
                  <div className="absolute top-4 right-4 z-10 bg-black/70 backdrop-blur-xs text-emerald-300 border border-emerald-500/30 text-[10px] font-mono px-2 py-0.5 rounded-md font-bold">
                    IMG #{idx + 1}
                  </div>
                  ...
                  <div className="relative rounded-lg overflow-hidden bg-slate-950 aspect-4/3 flex items-center justify-center border border-slate-800/80 group-hover:border-slate-700 transition-colors">
                    <img
                      src={imgUrl}
                      alt={`Extracted Telegram Image ${idx + 1}`}
        ...
        >
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-800 pb-2">
              <span className="text-xs font-bold text-slate-200">텔레그램 원본 고화질 이미지 미리보기</span>
              <button
                type="button"
                onClick={() => setLightboxUrl(null)}
